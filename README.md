...
| `/strix-onboard` | onboarding | Take a new client organization from no configuration to its first **independently verified** governed action: tenant, systems, governed capabilities, policies and approval routes, adapters and credentials (by reference only), connectivity validation, one governed smoke test through the real decision path, then independent proof verification. Readiness is derived from the verified proof, never asserted — see [`skills/strix-onboard/`](skills/strix-onboard/). |
